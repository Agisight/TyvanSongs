...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    range = [as.string rangeOfString: @"404 ошибка" options:0 range:range];
...
- (BOOL) isMatching:(NSString *)newUrl
{
    ...
    request.predicate = [NSPredicate predicateWithFormat:@"url = %@", newUrl];
    NSError *error;
...
- (void)insertNewObject:(NSAttributedString *) string {
    ...
    if (![[self hozakTrim:ats.string] hasPrefix:@"Warning"] && ![self isMatching:url])
    {
        NSManagedObjectContext *context = [_mvc.fetchedResultsController managedObjectContext];
        NSEntityDescription *entity = [[_mvc.fetchedResultsController fetchRequest] entity];
        ...
        //        NSLog(@"LoadedContent %@", ats.string);
        ...
        index = twinString;
        if (twinString!=nil) {
            NSString *audio_url = [NSString stringWithFormat:@"http://tyvan.ru/TyvanSongs/mp3/audio_%@.mp3", twinString];
