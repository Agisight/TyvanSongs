...
-(void)listenFeedbackUpdatesWithBlock:(feedbackBlock)block andFinishedBlock:(finishedBlock)finishedBlock {
    ...
    _feedbackTimer = [NSTimer scheduledTimerWithTimeInterval:updateRate block:^{
        ...
        welf.currentItem.timePlayed = (int)CMTimeGetSeconds(welf.player.currentTime);
        
        if (block) {
            
            block(welf.currentItem);
        }
        
        if (welf.statusDictionary[AFSoundStatusDuration] == welf.statusDictionary[AFSoundStatusTimeElapsed]) {
            
            [welf.feedbackTimer pauseTimer];
            
            welf.status = AFSoundStatusFinished;
